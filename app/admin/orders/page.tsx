          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-gradient-to-r from-slate-800 to-slate-900 rounded-xl flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-slate-800 focus:border-transparent transition-all"
              />
            </div>

            {/* Status Filter */}
            <div className="relative">
              <FiFilter className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
              <select
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value)}
                className="w-full pl-12 pr-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-slate-800 focus:border-transparent appearance-none transition-all"
              >
                ...
                {filteredOrders.map((order) => (
                  <tr key={order._id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-mono font-medium text-slate-900">#{order._id.slice(-8)}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-slate-900">{order.userId?.name || 'N/A'}</div>
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-900 capitalize font-medium">{order.paymentMethod}</div>
                      {order.mpesaCode && (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-900 text-white rounded-xl hover:bg-slate-800 text-sm font-medium transition-all shadow-sm hover:shadow-md"
                      >
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="sticky top-0 bg-gradient-to-r from-slate-800 to-slate-900 text-white px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-300">#{selectedOrder._id.slice(-8)}</p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <label className="text-sm font-semibold text-slate-600 uppercase tracking-wide">Order Status</label>
                  </div>
                ...
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <label className="text-sm font-semibold text-slate-600 uppercase tracking-wide">Payment Status</label>
                  </div>
                ...
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <label className="text-sm font-semibold text-slate-600 uppercase tracking-wide">Verification</label>
                  </div>
                  <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-sm font-semibold ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gradient-to-br from-slate-50 to-white rounded-xl p-5 border border-slate-200">
                  ...
                  <div className="space-y-2">
                    <p className="text-lg font-semibold text-slate-900">{selectedOrder.userId?.name || 'N/A'}</p>
                    <p className="text-sm text-slate-600">{selectedOrder.userId?.email || 'N/A'}</p>
                  </div>
                </div>
                <div className="bg-gradient-to-br from-slate-50 to-white rounded-xl p-5 border border-slate-200">
                  <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                  ...
                  <div className="space-y-2">
                    <p className="text-sm text-slate-600">Method: <span className="font-semibold text-slate-900 capitalize">{selectedOrder.paymentMethod}</span></p>
                    {selectedOrder.mpesaCode && (
                      <p className="text-sm text-slate-600">M-Pesa Code: <span className="font-mono font-semibold text-slate-900">{selectedOrder.mpesaCode}</span></p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-slate-50 to-white rounded-xl p-6 border border-slate-200">
                ...
                <div className="space-y-3">
                  {selectedOrder.items.map((item, index) => (
                    <div key={index} className="flex justify-between items-start p-4 bg-white rounded-xl border border-slate-200 hover:border-slate-300 transition-colors">
                      <div className="flex-1">
                        <p className="font-semibold text-slate-900">{item.productId?.title || item.title || 'Product'}</p>
                        <p className="text-sm text-slate-600 mt-1">Quantity: <span className="font-medium">{item.quantity}</span></p>
                        {item.buybackEligible && (
                          <div className="mt-2">
                            {item.buybackRequested ? (
                              ...
                              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-slate-100 text-slate-700 text-xs font-semibold rounded-full border border-slate-200">
                                ✓ Buyback Eligible
                              </span>
                            )}
                          </div>
                      ...
                      <p className="font-bold text-slate-900 text-lg">{formatPrice(item.price * item.quantity)}</p>
                    </div>
                  ))}
                </div>
                <div className="mt-5 pt-5 border-t border-slate-200 flex justify-between items-center">
                  <span className="text-lg font-bold text-slate-900">Total Amount</span>
                  <span className="text-2xl font-bold bg-gradient-to-r from-slate-800 to-slate-900 bg-clip-text text-transparent">{formatPrice(selectedOrder.total)}</span>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-slate-50 to-white rounded-xl p-6 border border-slate-200">
                ...
                <div className="space-y-2">
                  <p className="font-semibold text-slate-900 text-lg">{selectedOrder.shippingAddress.fullName}</p>
                  ...
                  <p className="text-sm text-slate-600">{selectedOrder.shippingAddress.city}, {selectedOrder.shippingAddress.county}</p>
                  {selectedOrder.shippingAddress.postalCode && (
                    <p className="text-sm text-slate-600">Postal Code: {selectedOrder.shippingAddress.postalCode}</p>
                  )}
                </div>
              </div>
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 py-3 rounded-xl font-semibold transition-all ${
                          isCurrentStatus
                            ? 'bg-slate-900 text-white shadow-md cursor-default border-2 border-slate-700'
                            : isValidTransition
                            ? 'bg-slate-700 text-white hover:bg-slate-800 shadow-sm hover:shadow-md'
                ...
                {selectedOrder.paymentMethod !== 'stk-push' && selectedOrder.paymentStatus === 'pending' && (
                  <div className="mt-5 p-5 bg-yellow-50 border-2 border-yellow-200 rounded-xl">
                    ...
                    <button
                      ...
                      disabled={updating}
                      className="w-full px-4 py-3 bg-green-600 text-white rounded-xl hover:bg-green-700 font-semibold disabled:bg-slate-400 transition-all shadow-sm hover:shadow-md"
                    >
